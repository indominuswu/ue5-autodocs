...
- User-facing: Yes - Provides the “Write To Niagara Data Channel” PCG node so authors can push PCG attributes into `UNiagaraDataChannelAsset` variables for Niagara/Blueprint/CPU/GPU consumers.
...
- `UPCGWriteToNiagaraDataChannelSettings` / `FPCGWriteToNiagaraDataChannelElement`  
  ...
  - Key properties: `DataChannel` (soft ref), `NiagaraVariablesPCGAttributeMapping` (name-to-attribute mapping), `bVisibleToGame`, `bVisibleToCPU`, `bVisibleToGPU`, `bSynchronousLoad`.  
