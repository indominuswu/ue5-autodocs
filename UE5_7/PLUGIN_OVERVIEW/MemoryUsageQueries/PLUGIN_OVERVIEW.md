...
- User-facing: Yes - Developers enable it for profiling and call `MemoryUsageQueries::GetMemoryUsage*`/`GetDependenciesWithSize*` (configured via `UMemoryUsageQueriesConfig`) to report package/dependency memory usage.
